...
**变量是用户定义的一个命名的内存段**。每个变量都只存储特定类型的数据，也就是说，每个变量都有一个类型，定义了可以存储的数据类别。每个基本类型都使用唯一的 **类型名称** 来标识。请注意，类型名称可能是由一个或多个 **关键字** 组成
...
在大部分情形下，这三种方式是等效的。但是，当发生 **窄化类型转换**(narrowing conversion) 时，初始化列表形式(花括号形式)会更加安全，因为花括号形式禁止这种转换的发生，至少编译器也会发出警窄化类型转换

```cpp
int banana_count {7.5}; // 错误：发生窄化类型转换，编译器禁止这种转换

int tangerine_count = 5.3; // OK: 截断为整数。编译器可能会发出警告提示
```
...
花括号（`{}`）形式的初始化器是 C++11 标准引入的。除了禁止窄化类型转换外，还能以相同的方式初始化所有变量，因此也称为 **统一初始化器**

:::

在定义变量时，**可以在一条语句中定义和初始化多个变量**。例如
...
**最好是在单个语句中只定义每个变量**。这会提高代码的可读性，还可以注释每个变量的作用

:::

基本类型的变量可以非常方便的将变量值写入到标准输出。下面的程序演示这一点

```cpp title="variable_int.cpp"
// variable_int.cpp
// Output the variable of type int to the standard output
...
在不同的应用场景下可能需要不同范围的整数。例如，记录一个人的年龄，通常只需一个八位的位模式，即 $1$ 字节；因为 $1$ 字节可以表示非负整数的范围为 $0 \sim 255$。如果使用 `int` 类型，就会浪费 $3$ 字节的内存。为了适应这种变化，C++ 标准提供了 **整型限定符** 用于限定整型 `int` 的范围。目前 C++ 标准支持下表列出的几个整型限定符
...
默认情形下，类型 `int` 是 `signed` 的。 也就是说 `signed` 是可以省略的。此外，这些限定符只用于整型 `int`。换句话说， 在有限定符的情形下， `int` 也是可以省略的
...
字面值是直接写在程序源代码中，而无需使用变量存储的值。例如，语句 `const int toe_count {10};` 中的 `10` 就是字面值

:::

请注意：字面值是有类型的。这里我们首先介绍整型字面值。C++ 支持 $4$ 种整型字面值，分别是 **十进制字面值**、**十六进制字面值**、**八进制字面值** 和 **二进制字面值**
...
运算由运算符定义，例如，运算符 `+` 执行加法运算，运算符 `*` 执行乘法运算。运算符操作的对象称为操作数。在表达式 `2 * 3` 中，操作数就是 2 和 3.乘法运算需要两个操作数，所以称为 **二元运算符**。只需要一个操作数的运算符称为 **一元运算符**。表达式 `-width` 中，`-` 就是一元负号，表示对操作数 `width` 的符号取反。换句话说，如果 `width < 0`，那么 `-width > 0`
...
这是算术运算符都是二元运算符，其工作方式与我们期望的大致相同。不同，对于运算符 `/` 和 `%` 需要做特殊说明。在 C++ 中，整数除法相对特殊。**当运算符 `/` 用于两个整数操作数时，其运算结果总是整数**。例如，假设进行下面的运算：
...
取模运算符 `%` 是对整数除法的补充，它提供了一种在整数除法运算后获取其余数的方式。取模运算的是根据公式 $\text{remainder} = \text{dividend} - \lfloor \text{dividend} \div \text{divisor} \rfloor$ 进行计算的。使用这个公式，很容易推断出负操作数使用取模运算的结果
...
首先，变量 `x` 被初始化为 5，所以表达式 `x + 1` 的值是 6。这个结果被重写到 `x` 命名的内存中。下面是一个示例程序：该程序执行一个距离换算操作
...
int main() {
    ...
    const unsigned int feet_per_yard{ 3 };    // 1 码 = 3 英尺
    ...
    // 将  n 英寸转换为 x 码 y 英尺 z 英寸
    std::cout << "Enter a distance in inches: ";
    std::cin >> total_inches;

    feet = total_inches / inches_per_foot;   // total_inches 中有 feet 英尺
    inches = total_inches % inches_per_foot; // 剩余的英寸
    yards = feet / feet_per_yard;            // feet 个英尺中有 yards 码
    feet = feet % feet_per_yard;             // 剩余的英尺
...
在 `main()` 中的第一条语句定义了 $3$ 个整型变量，并把它们初始化为 $0$。它们的类型都是 `unsigned int`，这是因为距离是非负的。然后，定义了 `feet_per_yard` 和 `inches_per_foot` 分别用于将码、英尺和英寸统一转换为英寸；这两个变量的值是固定的，所以将它们声明为 `const`
...
有时候我们需要存储一些小数，例如银行卡账户的余额。现在，我们来学习在 C++ 中如何声明浮点数类型的变量。当希望使用非整数值时，可以使用 **浮点类型的变量**。浮点变量的数据类型有 $3$ 中
...
C++ 标准并没有规定精度和数值范围，所以这取决编译器实现，也取决于计算机使用的处理器类型及其使用的浮点数表示方法。不过，C++ 标准要求 `long double` 提供的精度不小于 `double` 类型提供的精度，`double` 类型提供的精度不小于 `float` 类型提供的精度。现在，几乎所有的编译器和计算机架构都按照 IEEE 754 binary 规定的浮点数和浮点数运算。
...
与 `import` 语句不同，`#include` 指令后不能有分号。另外，不要忘记前面的符号 `#`
...
IEEE 浮点数标准定义了几个特殊值。下表列出来特殊值 `infinity` 和 `NaN` 的操作
...
其中，方括号表示这些格式说明符是可选的。**如果不指定格式说明符，那么冒号也是不需要的**。格式说明符告诉 `std::format()` 要如何格式化对应的数据。例如，要调整浮点数的精度，可以在花括号中间填入 `:.精度` 接口
...
`char` 类型的变量主要用于存储单个字符的编码。在 C++ 中，`char` 类型占用 $1$ 字节，虽然 C++ 标准没有指定用于表示基本字符集的字符编码，所以原则上是由编译器指定的，但一般使用 ASCII 编码
...
此外，还允许使用 **字符转义序列**，用于从键盘输入那些无法从键盘输入的字符。转移字符序列以反斜线(`\`) 开头。下表列出了 C++ 标准中规定的字符转义序列
...
此外，有些字符无法从键盘输入，并且也不存在于字符转义序列中。对于这种字符可以使用 **八进制** 或在 **十六进制** 表示的字符编码
...
`char` 类型对于使用拉丁文的国家完全够用。但是，如果要同时使用多种语言的字符，或者要处理非拉丁文的字符，$1$ 字节就完全不够用了
...
`char8_t` 类型是 C++20 中引入的。因此，大多数老式的代码和库还是使用类型 `char` 表示 UTF-8 编码的字母
...
为了可移植性，请遵循一个准则：**auto 的初始化器不应该使用初始化列表，而应该使用 C 风格初始化器或者函数式初始化器**
