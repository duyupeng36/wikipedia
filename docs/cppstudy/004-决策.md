...
在 `switch` 语句块中的变量定义必须能在执行该语句块时访问，不能通过跳转到相同作用域内位于变量声明之后的 `case` 语句来绕过定义，否则代码编译就会报错。来看如下示例代码
...
switch (test)
{
    ...
    case 1:
        int j {2};  // 不合法：直接跳转到 case 3 或者 default 标签执行，跳过了该变量的实际定义位置
    ...
    default:
        int n {5};  // 合法的变量定义。但是，如果 default 之后还有其他 case 标签，那么变量 n 的声明也是非法的
